var userSubmit = document.querySelector("#user-submit");

var userInput = document.querySelector("#user-input");	

var toDoList = document.querySelector("#to-do-list");

function addToDo(event){
	event.preventDefault();		//prevents refreshing

	if(userInput.value === ""){
		return false;
	}

	toDoList.innerHTML = "<li><i class=\"fa fa-window-close close-to-do\" aria-hidden=\"true\"></i>" + userInput.value + "</li>" + toDoList.innerHTML; //creates list item element on the html file
	/*
	var li = document.createElement("li");
	var icon = document.createElement("i");
	li.innerHTML = userInput.value + "  <i class=\"fa fa-window-close\" aria-hidden=\"true\"></i>"; 
	toDoList.prepend(li);
	*/
	userInput.value = ""; //clears out the input box
}

function removeToDo(event){
	if (event.target.classList.contains('close-to-do')){
		var li = event.target.parentElement;
		toDoList.removeChild(li);
	}
}

toDoList.addEventListener("click", removeToDo, false);

userSubmit.addEventListener("click", addToDo, false);